...
class RuleEngineController extends AbstractController
{
    ...
    try {
      ...
      if ($status['progress'] < 100) {
        for ($i = 0; $i < 6; $i++) {
          $status = $callApiService->get_status($token, $ciUploadId);
          if ($status['progress'] == 100) {
            $this->completed_response($status);
            break;
          }
          sleep(5);
        }
      } else {
        $this->completed_response($status['vulnerabilitiesFound']);
  ...
  public function completed_response($vulnerabilitiesFound)
  {
    $vulnerabilities_value = $this->getParameter('app.vulnerabilities_value');
    if ($vulnerabilitiesFound > $vulnerabilities_value) {
      $message = "The number of vulnerabilities found is" . $vulnerabilitiesFound;
