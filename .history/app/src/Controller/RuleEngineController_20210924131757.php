...
class RuleEngineController extends AbstractController
{
    ...
    try {
      ...
      if ($status['progress'] < 100) {
        for ($i = 0; $i < 6; $i++) {
          $status = $callApiService->get_status($token, $ciUploadId);
          if ($status['progress'] == 100) {
            return $this->completed_response($status, $notifyService);
            break;
          }
          sleep(5);
        }
      } else {
        $this->completed_response($status, $notifyService);
      }
    } catch (\Exception $e) {
      return new JsonResponse(["message" => 'Something went wrong!'], $e->getCode());
    }
   ...
   * @return void
   */
  public function completed_response($status, $notifyService)
  {
