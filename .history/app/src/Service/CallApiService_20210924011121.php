...
class CallApiService
{
  ...
  public function upload_file($token, $filePath, $repositoryName, $commitName)
  {
    ...
    $response = $this->client->request(
      'POST',
      $this->api_url . 'open/uploads/dependencies/files',
      [
        'headers' => array_merge(
  ...
  public function conclude_file($token, $ciUploadId)
  {
    ...
    $response = $this->client->request(
      'POST',
      $this->api_url . 'open/finishes/dependencies/files/uploads',
      [
        'headers' => [
  ...
  public function get_status($token, $ciUploadId)
  {
    $response = $this->client->request(
      'GET',
      $this->api_url . 'https://app.debricked.com/api/1.0/open/ci/upload/status?ciUploadId=' . $ciUploadId,
      [
        'headers' => [
          'Accept' => 'application/json',
          'Authorization' => 'Bearer ' . $token
